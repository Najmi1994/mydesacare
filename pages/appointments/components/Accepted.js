...
import { useAppointment, useBookingUpdate } from 'src/actions/appointment';
import ActionInvited from "./ActionInvited";
...
export default function Accepted() {
    ...
    const { data, isLoading, isSuccess, isFetching, isError, error } = useAppointment('2',page);
    ...
    return (
        ...
        <ToastContainer />
            <div>
                    ...
                    <table className="w-full text-sm text-left text-gray-500 hidden">
                        ...
                        {data?.data.map((booking, index) => (
                            <tr className="bg-white border-b hover:bg-gray-50 text-center">
                                ...
                                <td className="px-6 py-4">{booking.meeting_link ? booking.meeting_link : 'Please Invite Meeting Link'}</td>
              ...
              {showModal != false ? (
                <>
                    <ActionInvited onHandlerModal={onHandlerModal} dataBooking={dataBooking} />
                </>
                ) : (
                    ""
                )}
